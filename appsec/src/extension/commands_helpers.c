...
static void _command_process_telemetry_metrics(mpack_node_t metrics);
 ...
 *    1: [if block/redirect parameters: (map)],
 *    2: [if block/redirect/record: appsec span data (array of strings: json
 *        fragments)],
 *    3: [force keep: bool]
 *    4: [meta: map]
 *    5: [metrics: map]
 *    6: [telemetry metrics: map string ->
 *       array(array(value: double, tags: string)])
 * )
 */
#define RESP_INDEX_VERDICT 0
#define RESP_INDEX_ACTION_PARAMS 1
#define RESP_INDEX_APPSEC_SPAN_DATA 2
#define RESP_INDEX_FORCE_KEEP 3
    ...
    mpack_node_t root, void *unspecnull _ctx)
{
    ...
    mpack_node_t verdict = mpack_node_array_at(root, RESP_INDEX_VERDICT);
    ...
    if (dd_mpack_node_lstr_eq(verdict, "block")) {
        ...
        _command_process_block_parameters(
            mpack_node_array_at(root, RESP_INDEX_ACTION_PARAMS));
        dd_tags_add_blocked();
    ...
    if (mpack_node_array_length(root) >= RESP_INDEX_TELEMETRY_METRICS + 1) {
        _command_process_telemetry_metrics(
...
static void _command_process_telemetry_metrics(mpack_node_t metrics)
{
    ...
    for (size_t i = 0; i < mpack_node_map_count(metrics); i++) {
        ...
        for (size_t j = 0; j < mpack_node_array_length(arr_value); j++) {
            ...
            double dval = mpack_node_double_strict(dval_node);
    ...
    zend_string *_Atomic *nonnull zstr, const char *nonnull str, size_t len)
{
    ...
    zend_string *zstr_new = zend_string_init(str, len, 0);
...
#define HANDLE_METRIC(name, type)                                              \
    do {                                                                       \
        if (key_len == sizeof(name "") - 1 &&                                  \
            ...
            zend_string_free(tags_zstr);                                       \
            return;                                                            \
        }                                                                      \
    } while (0)

    HANDLE_METRIC("waf.requests",
        DDTRACE_METRIC_TYPE_COUNT);
    HANDLE_METRIC("waf.updates",
        DDTRACE_METRIC_TYPE_COUNT);
    HANDLE_METRIC("waf.init",
        DDTRACE_METRIC_TYPE_COUNT);
    HANDLE_METRIC(
        "waf.input_truncated",
        DDTRACE_METRIC_TYPE_COUNT);
    ...
    HANDLE_METRIC(
        "remote_config.first_pull", DDTRACE_METRIC_TYPE_GAUGE);
    HANDLE_METRIC("dd.appsec.remote_config.requests_before_running",
        DDTRACE_METRIC_TYPE_COUNT);
    HANDLE_METRIC(
        "remote_config.last_success", DDTRACE_METRIC_TYPE_GAUGE);

    mlog_g(
        dd_log_debug, "Unknown telemetry metric %.*s", (int)key_len, key_str);
}
